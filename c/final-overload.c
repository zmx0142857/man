...
#define sizeof_overload(A)\
	((sizeof(A) == sizeof(float))?sizeof_overload_float(A):\
	 ...
	 (sizeof(A) == sizeof(long double))?sizeof_overload_longdouble(A):\
	 (void)0)
...
#define gcc_overload(A)\
	...
	__builtin_types_compatible_p(typeof(A), struct s1),\
	gcc_overload_s1(*(struct s1 *)&A),\
	__builtin_choose_expr(\
	__builtin_types_compatible_p(typeof(A), struct s2),\
...
#define gcc_type_overload(A)\
	gcc_type_overload_aux(\
		...
		__builtin_types_compatible_p(typeof(A), struct s2) * 2, A)
