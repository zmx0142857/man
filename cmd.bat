...
扇区
	...
	又叫分区. 同一块磁盘可以有多个分区. 跨分区的文件访问存在一些限制.
	一般不宜设定过多的分区, 一个系统分区和一个用户文件分区已经足够.
...
%0 %1 ... %9 %*
	...
	再运行 cmd, 输入 c:\test.bat "/a" /b /c /d, 可以参照 call /? 或 for /?
	看出每个参数的含义. 注意这里可以对文件进行日期比较和大小比较. 
...
set [variable][=value]
	...
	语句生效, 可以启用 "延缓环境变量扩充", 并用 ! 而不是 % 来引用变量.
	另外, 为使 "延缓环境变量" 只在局部有效, 使用了命令 setlocal:
...
if [condition] (
	...
	if 语句中, 判断 errorlevel 有 3 种写法. 应先判断较大的 errorlevel:
