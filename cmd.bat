...
echo %errorlevel%	:: 上一条命令成功执行时, 返回 0, 否则返回非 0 值.

:::: cd 切换工作目录 (change directory) ::::::::::::::::::::::::::::::

cd					:: 显示当前目录, 相当于 unix 的 "pwd"
cd subdir			:: 进入当前目录下名为的 subdir 子目录, 不存在时报错
					:: 此时 %errorlevel% 为 1
cd ..				:: 进入上级目录
cd \				:: 进入当前分区的根目录
cd /d				:: 跨磁盘分区切换目录, 比如当前在 C 盘,
					:: 要切换到 D 盘时, 就需要 /d 参数
...
@					:: 在批处理文件中, 用在一行开头, 取消当前行的回显

"回显" 是指运行批处理脚本时, 除了显示运行结果, 还显示脚本里的命令.
默认情况下回显是开启的.
...
echo				:: 查看当前回显是否被打开
echo on				:: 打开回显, 相当于 unix 下 "bash -x <文件名>"
@echo off			:: 关闭回显. 一般写在批处理文件开头

:::: dir 列出文件和子目录, 相当于 unix 的 ls :::::::::::::::::::::::::

dir					:: 列出当前目录中的文件和子目录
					:: 如果指定的目录或文件不存在, 则置 errorlevel=1.
dir /a				:: 同 dir, 但包括隐藏文件和系统文件
dir /b				:: 同 dir, 但不显示详细信息
dir c:\ /a:d		:: 显示 C 盘根目录中的子目录
dir d:\mp3 /p	  	:: 逐屏显示 d:\mp3 目录里的文件, 只显示文件名
dir *.exe /s		:: 显示当前目录和子目录里所有的 .exe 文件

使用 dir 可以看到, 每个文件夹都有 2 个子目录 "." 和 "..".
"." 代表当前目录, ".." 代表当前目录的上级目录. 

dir .				:: 显示当前目录中的文件和子目录
dir ..				:: 显示上级目录中的文件和子目录

通配符 "*" 代表任意字符串, 通配符 "?" 代表任意一个字符.  如:

	"c*.*" 代表以 "c" 开头的所有文件,
	"?.exe" 代表所有文件名是一个字符的 ".exe" 文件.

:::: md 创建目录 (make directory), 相当于 unix 的 mkdir ::::::::::::::
 
md abc				:: 在当前目录里建立子目录 "abc"
md d:\a\b\c			:: 如果 "d:\a\b" 不存在, 将会自动创建

:::: rd 删除目录 (remove directory), 相当于 unix 的 rmdir ::::::::::::
...
:::: ren 重命名 (rename), 相当于 unix 的 mv ::::::::::::::::::::::::::
 
ren 1.txt 2.bak		:: 把 1.txt 更名为 2.bak
ren *.txt *.ini		:: 把当前目录里所有.txt文件改成.ini文件
ren d:\temp tmp		:: 重命名文件夹

:::: cls 清屏 (clear screen), 相当于 unix 的 ctrl-l ::::::::::::::::::

:::: type 把文本文件内容输出到控制台, 相当于 unix 的 cat :::::::::::::
...
:::: date, time 时间和日期, unix 中的 time 是程序计时工具而非时间 ::::

date				:: 显示当前日期, 并提示输入新日期, enter 略过输入
date /t				:: 显示当前日期
time				:: 显示当前时间, 并提示输入新时间, enter 略过输入
time /t		  		:: 显示当前时间
echo 12:00 | time	:: 将时间修改为 12:00, 要求管理员权限.
...
:::: find 查找 (外部命令), unix 中的对应物是 grep ::::::::::::::::::::

find "abc" c:\test.txt
					:: 在 c:\test.txt 文件里查找含 'abc' 的行.
					:: 如果找不到, 置 errorlevel=1
	/i				:: 忽略大小写
	/c				:: 显示成功匹配的行数

所谓 "外部命令", 即 cmd.exe 本身不支持, 而通过调用其他程序来实现的命令.
...
将前一个命令的执行结果作为后一个命令的输入 (如果后一个命令需要键盘的输入)

	dir *.* /s/a | find /c ".exe"

表示先执行 dir 命令, 将其输出的结果传递给 find 命令.
该命令用于输出当前文件夹及所有子文件夹里的 .exe 文件, 并计数.
...
copy c:\test.txt f: > nul
					:: 拷贝文件, 不显示 "文件复制成功" 的提示信息.
					:: 但如果 f 盘不存在, 还是会显示出错信息
...
一般用于 date time label 等需要等待输入的命令, 如: 
	...
	date < temp.txt :: 假定 temp.txt 的内容是 2015-01-01
...
再运行cmd, 输入 c:\test.bat "/a" /b /c /d, 可以参照 call/? 或 for/?
...
因为在读取 if 语句时已经将所有 %p% 替换为 aaa. 这里的 "替换",
在 /? 帮助里就是指 "扩充", "环境变量扩充" 可以启用 "延缓环境变量扩充",
用 ! 来引用变量, 即 !变量名!: 
...
notepad.exe 打开, 而是先判断 .txt 属于 txtfile "文件类型",
再调用 txtfile 关联的命令行
